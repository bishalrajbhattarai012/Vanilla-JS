...
const favLists = <HTMLDivElement>document.querySelector("#favoriteList");
...
document.body.addEventListener("click", (e: Event) => {
  ...
  if (target.matches("#generateBtn")) {
    const palette = [getRandomHexColor(), getRandomHexColor(), getRandomHexColor(), getRandomHexColor()];
  ...
  if(target.matches(".fav")){
    ...
    const favColors = JSON.parse(localStorage.getItem("color") || "[]")
